...
def get_updated_csv(main_folder_path: str, video_name: str, csv_type: str):
...
def visualize_results(
    main_folder_path: str, clip_name: str = None, key_word: str = "camera"
) -> None:
    ...
    df_bboxes = get_updated_csv(main_folder_path, clip_name, "bbox")
    df_labels = get_updated_csv(main_folder_path, clip_name, "sequential")
    if df_bboxes is None and df_labels is None:
        raise FileNotFoundError(f"No CSV file found for.")

    # Get videos by unique video names in the CSV file
    camera_name = df_bboxes[key_word].unique()[0] if df_bboxes is not None else []
    ...
    df_video_labels = (
        df_labels[df_labels[key_word] == camera_name.lower()]
...
def control_video_playback(play, frame_id, total_frames):
    ...
    # Get key press
    key = cv2.waitKeyEx(1) if play else cv2.waitKeyEx(0)
    ...
    interval = min(interval, 2**6)  # Limit interval to a maximum of 100ms
...
def draw_gesture_labels(frame, df_video_labels, frame_id, x1, y1, pedestrian_id):
    ...
    label_values = {value: row[key] for key, value in LABEL_KEYS.items() if key in row}
...
def draw_info(frame, clip_name, frame_id, interval, camera_name=None):
    """Draw the video name, frame number, and interval on the frame."""

    info = [
        f"Video: {clip_name}" + (f" ({camera_name})" if camera_name else ""),
        f"Frame: {frame_id}",
        f"Interval: {interval}",
    ]
...
def visualize_video(video_path, df, df_video_labels=None):
    ...
    clip_name = split_video_file[-2]
    camera_name = split_video_file[-1].split(".")[0]

    # Filter the DataFrame for the current video
    df_video = df[df["camera"] == camera_name]
    if df_video.empty:
        print(f"Error: No data found for video {camera_name} in the CSV file.")
        return
    ...
    while cap.isOpened():
        ...
        if show_hud:
            frame = draw_pedestrians(
                frame, df_video, frame_id, width, height, df_video_labels
            )
            frame = draw_info(frame, clip_name, frame_id, interval, camera_name)
...
if __name__ == "__main__":
    ...
    # Example usage:
    """ 
    python scripts/visulize_bboxes.py --main_folder "../data/conflict_acted_navigation_gestures" --clip_name "video_00"
    """

    visualize_results(args.main_folder, args.clip_name)
